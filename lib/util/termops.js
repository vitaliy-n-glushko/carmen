...
function replaceTokenPermutations(tokenReplacer, query) {
    ...
    var queryPermutations = permutations(terms);
    var tokenPermutations = [query];
    console.log(queryPermutations);
    queryPermutations.forEach(function(queryPermutation) {
        queryPermutation.forEach(function(queryTerm) {
            tokenize(token.replaceToken(replacer, queryPermutation[queryTerm]));
        })
    });
