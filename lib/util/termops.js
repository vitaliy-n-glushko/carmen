...
function getIndexableText(replacer, doc) {
    ...
    function add(texts) {
        var text = texts.join(' ');
        indexableText.push(text);
    }
    console.log('indexabe', indexableText);
...
function replaceTokenPermutations(tokenReplacer, query) {
    ...
    var permuteTokens = JSON.parse(JSON.stringify(permutations(Object.keys(tokenReplacer))));
    permuteTokens.forEach(function(pT) {
        queryWithTokenPermutations.push(token.replaceToken(tokenReplacer.slice(parseInt(pT[0]),parseInt(pT[pT.length - 1]) + 1), query));
    });
