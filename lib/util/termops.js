...
function tokenize(query, lonlat) {
    ...
    var normalized = token.normalize(query)
...
function decollide(replacer, doc, subq) {
    ...
    for (var i = 0; i < length; i ++) {
        ...
        if (code && mun.hasLanguage(code[1]) &&
            doc.properties[keys[i]]) texts = texts.concat(doc.properties[keys[i]].split(','));
    }

    var tokenizedTexts = [];
    for (var t = 0; t < texts.length; t++) {
        tokenizedTexts = tokenizedTexts.concat(token.getQueryTokenVariations(replacer, tokenize(texts[t])));
    }

    var a = tokenizedTexts.length;
    var fails = 0;
    while (a--) {
        var text = unidecode(tokenizedTexts[a].join(' ').toLowerCase()).toLowerCase().trim();
        var textHash = {
            32: true, // ' ' for spaces
        };
        var b = text.length;
    ...
    return fails !== tokenizedTexts.length;
}
