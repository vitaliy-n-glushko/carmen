...
Cache.prototype.updateFreqs = function(getter, ids, freq, callback) {
    var cache = this;
    cache.loadall(getter, 'freq', ids, function(err) {
        if (err) return callback(err);
...
Cache.prototype.setGridParts = function(getter, data, callback) {
    var ids = Object.keys(data);
    var cache = this;
    cache.loadall(getter, "grid", ids, function(err) {
        if (err) return callback(err);
        for (var i = 0; i < ids.length; i++) {
            var id = ids[i];
...
Cache.prototype.store = function(source, callback) {
    var tasks = [];
    var cache = this;
    ...
    function loadTerm(type) {
        console.log(type, cache.list(type).length);
