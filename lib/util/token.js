...
var createReplacer =  module.exports.createReplacer = function(tokens, reverse) {
    var replacers = [];
...
module.exports.tokenReplacerFilter = function(tokens, query) {
    var matches = [];
    matches = tokens.filter(function(token) {
        return ((!token.named && token.from.test(query)) || (token.named && XRegExp(token.from).test(query)))
    });
    return matches;
};
