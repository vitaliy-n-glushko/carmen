...
var createReplacer =  module.exports.createReplacer = function(tokens) {
    ...
    for (var token in tokens) {
        ...
        for (var u = 0; u < 2; u++) {
            ...
            //console.log('parseFrom', parsedFrom);
...
module.exports.createReverseReplacer = function(tokens) {
    var replacers = [];

    for (var token in tokens) {
        var from = tokens[token]; 
        var to = token;
        var reverse = {};
        var parsedFrom = new XRegExp(from);

        reverse.named = (parsedFrom.xregexp.captureNames !== null);

        if (reverse.named && parsedFrom.xregexp.captureNames.some(function(captureName) { return captureName === null; }))
        throw new Error('Cannot process \'%s\'; must use either named or numbered capture groups, not both', from);

        if (reverse.named) {
            reverse.from = new XRegExp('(?<tokenBeginning>\\W|^)' + from + '(?<tokenEnding>\\W|$)', 'gi');
            reverse.to = '${tokenBeginning}' + to + '${tokenEnding}';
        }
        else {
            reverse.from = new RegExp('(\\W|^)' + from + '(\\W|$)', 'gi');
            var groupReplacements = 0;
            to = to.replace(/\$(\d+)/g, function(str, index) { groupReplacements++; return '$' + (parseInt(index)+1).toString();});
            reverse.to = '$1' + to + '$' + (groupReplacements + 2).toString();
        }
        replacers.push(reverse);
    }
    return replacers;
}
