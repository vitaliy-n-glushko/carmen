...
var MPCache = function(id, size, source, geocoder_address) {
    ...
    this.size = size;
    this.source = source;
    this.geocoder_address = geocoder_address;
    this.cpus = require('os').cpus().length;

    this.workers = [];
}

MPCache.prototype._spawnWorkers = function(callback) {
    for (var i = 0; i < this.cpus; i++) {
        ...
        this.workers[i].on('exit', function(code) {
            if (!code) return;
            console.warn('Cache worker exited with ' + code);
            process.exit(code);
        });
    ...
    this.runAll('init', {}, {
        ...
        source_uri: "mbtiles://" + this.source.filename,
        geocoder_address: this.geocoder_address
    }, callback);
}
...
MPCache.prototype.runAll = function(func, optsPerChild, optsShared, callback) {
    ...
    if (!this.workers.length) {
        this._spawnWorkers(function() {
            cache.runAll(func, optsPerChild, optsShared, callback);
        });
        return;
    }
...
MPCache.prototype.teardown = function(callback) {
    this.runAll("exit", {}, {}, callback);
}
