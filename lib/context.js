...
module.exports = function(geocoder, lon, lat, options, callback) {
    ...
    for (var index_ids_it = 0; index_ids_it < index_ids.length; index_ids_it++) {
        ...
        if (lat >= bounds[1] && lat <= bounds[3] && lon >= bounds[0] && lon <= bounds[2]) {
            ...
            if (limit) {
                if (options.types[0] !== source.type || (options.landmark && !source.landmark)) continue;
    ...
    function contextFormatter(err, res) {
        ...
        for (var i = 0; i < res.length; i++) {
            ...
            if (toFilter) {
                ...
                //Filter context results by landmark
                if (options.landmark && !source.landmark) filter = false;
