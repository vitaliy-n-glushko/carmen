...
function spatialmatch(query, phrasematches, options, callback) {
    ...
    options.scoreAbove = allowedScores(stacks, options);
...
function allowedScores(stacks, options) {
    var score = {};
    if (!options.scoreAbove) return stack;
    for (var i = 0; i < stacks.length; i++) {
        var stack_maxidx = 0;
        for (var j = 0; j < stacks[i].length; j++) {
            stack_maxidx = Math.max(stack_maxidx, stacks[i][j].idx);
        ...
        if (options.allowed_idx[stack_maxidx]) {
            score[stack_maxidx] = scoreAbove * stacks[i].scorefactor;
        }
    }

    options.scoreAbove = score;
    return options.scoreAbove;
}
