...
function dropFeature(geocoder, options, results, callback) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (qualifyResult([options.types, options.stacks, options.scoreAbove], [{func: typeMatch, arguments: [options.types, source.type]}, {func: stackMatch, arguments: [options.stacks, source.stack]}, {func: scoreMatch, arguments: [options.scoreAbove * 0.01 *   results[i][0].scorefactor, results[i][0].score * results[i][0].scorefactor]}])) {
