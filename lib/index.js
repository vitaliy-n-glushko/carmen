...
function index(geocoder, from, to, options, callback) {
    ...
    to.startWriting(function(err) {
        ...
        to._db.run("pragma journal_mode=WAL", function() {});
        to._db.run("pragma busy_timeout=600000", function() {});
                ...
                if (!docs.length) {
                    ...
                    teardown(function() {
                        ...
                        store(to, function(err) {
                            ...
                            to.stopWriting(function() {
                                to._db.run("pragma journal_mode=DELETE", callback);
                            });
                        });
                    });
                } else {
...
function update(source, docs, zoom, callback) {
    ...
    source._geocoder.loadall(getter, 'freq', ids, function(err) {
        ...
        // increase our busy timeout so we don't conflict with workers

        indexdocs(docs, freq, zoom, source._geocoder.geocoder_tokens, source, updateCache);
    });
