...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    loadall(getter, 'freq', [1], function(err) {
        ...
        var results = [];
        subqueriesCollection.forEach(function(subquery) {
            ...
            while (l--) {
                ...
                subquery[l].weight = subquery[l].length / tokenizedCollection[l].length;
...
function deDuplicateSubqueries(subqueries) {
    ...
    for (var j = 0; j < subqueries.length; j++) {
        var subqueriesInner = subqueries[j];
        for (var i = 0; i < subqueriesInner.length; i++) {
            var item = subqueriesInner[i];
