...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    queries.forEach(function(query) {
        ...
        tokenizedCollection.push(tokenized);
    });
    ...
    loadall(getter, 'freq', [1], function(err) {
        ...
        subqueriesCollection.forEach(function(subquery, subQueryIndex) {
            ...
            while (l--) {
                ...
                subquery[l].weight = subquery[l].length / tokenizedCollection[subQueryIndex].length;
...
function deDuplicateSubqueries(items, key) {
    ...
    items.forEach(function(item, itemIndex) {
        item.forEach(function(itemInner, itemInnerIndex) {
            var k = key ? key.apply(itemInner) : itemInner;
            if (k in set) {
                if (item.length === 1) {
                    items.splice(itemIndex);
                } else {
                    console.log(item.splice(itemInnerIndex,1));
                }
            } else {
                set[k] = true;
            }
