...
module.exports = function phrasematch(source, query, options, callback) {
    ...
    queries.forEach(function(query) {
        ...
        subqueries = subqueries.concat(subqueries);
        ...
        subqueriesCollection.push(subqueries);
    });
    subqueriesCollection = deDuplicateSubqueries(subqueriesCollection);
    loadall(getter, 'freq', [1], function(err) {
        ...
        var results = [];
        subqueriesCollection.forEach(function(subquery, subQueryIndex) {
            var l = subquery.length;
            while (l--) {
                if (!source._dictcache.hasPhrase(subquery[l])) continue;
                // Augment permutations with matched grids,
                // index position and weight relative to input query.
                subquery[l].scorefactor = scorefactor;
                subquery[l].getter = getter;
                subquery[l].loadall = loadall;
                subquery[l].cache = source._geocoder;
                subquery[l].idx = source.idx;
                subquery[l].zoom = source.zoom;
                subquery[l].nmask = 1 << source.ndx;
                subquery[l].bmask = source.bmask;
                results.push(subquery[l]);
            }
        });
...
function deDuplicateSubqueries(items, key) {
    ...
    items.forEach(function(item, itemIndex) {
        indexMap[itemIndex] = [];
        item.forEach(function(innerItem, innerItemIndex) {
            var key = (innerItem.ender.toString() + innerItem.mask.toString());
            if (!(key in set)) {
                indexMap[itemIndex].push(innerItemIndex);
                set[key] = true;
            }
        });
    });
    ...
    for (key in indexMap) {
        if (indexMap[key].length) {
            deduped[key] = [];
            indexMap[key].forEach(function(innerItemIndex) {
                deduped[key].push(items[key][innerItemIndex]);
            });
        }
    };
