...
var cleanDocs = require('../index').cleanDocs;
...
/* define a local in-memory source */
inherits(WorkerSource, MemSource);
function WorkerSource(callback) {
    this._shards = {};
    this._grids = {};
    this._info = { maxzoom: 6 };
...
WorkerSource.prototype.putGeocoderData = function(type, shard, data, callback) {
    this._pending += 1;
    if (this._shards[type] === undefined) this._shards[type] = {};
    this._shards[type][shard] = data;
    return callback(null);
};
WorkerSource.prototype.drain = function() {
    data = this.serialize();
    this._shards = {};
    this._pending = 0;
    return data;
...
var flush = function() {
    process.send(source.drain());
}

process.on('message', function(data) {
    if (data.freq && data.zoom && data.geocoder_tokens && data.source_uri) {
        freq = data.freq;
        zoom = data.zoom;
        token_replacer = token.createReplacer(data.geocoder_tokens);

        source = new WorkerSource();
        ...
        toSend.docs = {length: patch.docs.length};

        process.send(toSend);

        var cuuid = guid();
        console.log(process.pid, "putting", cuuid);
        commitsPending++;
        feature.putFeatures(source, cleanDocs(source, patch.docs), function(err) {
            console.log(process.pid, "back from put", cuuid);
            if (err) return process.send(err);
            console.log(process.pid, "no error, attempting commit", cuuid);

            console.log(source._pending);
            if (source._pending > 100) {
                flush();
            }
