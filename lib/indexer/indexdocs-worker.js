...
var resolveWriteStarted, rejectWriteStarted;
var writeStarted = new Promise(function(fulfill, reject) { resolveWriteStarted = fulfill; rejectWriteStarted = reject; });
var shouldDie = false;
...
process.on('message', function(data) {
    if (data.freq && data.zoom && data.geocoder_tokens && data.source_uri) {
        ...
        source = new tilelive.protocols[uri.protocol](uri, function() {
            ...
            source.startWriting(function() {
                // increase the busy timeout from a second to ten minutes; no timeouts, damn it
                source._db.run("pragma journal_mode=WAL", function() {});
                source._db.run("pragma busy_timeout=600000", function() {});
                resolveWriteStarted();
            });
        });
    } else if (data == "exit") {
        ...
        writeStarted.then(function() {
            ...
            feature.putFeatures(source, cleanDocs(source, patch.docs), function(err) {
                ...
                source._commit && source._commit(function() {
                    commitsPending--;
                    if (commitsPending == 0 && shouldDie) {
                        exit();
                    }
                });
            });
