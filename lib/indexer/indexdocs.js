        ...
        if (docs[i].properties['carmen:addressnumber']) {
            for (c_it = 0; c_it < docs[i].properties['carmen:addressnumber'].length; c_it++) {
                ...
                for (addr_it = 0; addr_it < docs[i].properties['carmen:addressnumber'][c_it].length; addr_it++) {
                    var lon = parseFloat(parseFloat(docs[i].geometry.geometries[c_it].coordinates[addr_it][0]).toFixed(6));
                    var lat = parseFloat(parseFloat(docs[i].geometry.geometries[c_it].coordinates[addr_it][1]).toFixed(6));
                    feat = point([lon, lat], vectorProperties(docs[i].properties, true));
                    feat.id = docs[i].id;
        ...
        if (docs[i].properties['carmen:rangetype']) {
            for (c_it = 0; c_it < docs[i].geometry.geometries.length; c_it++) {
                ...
                for (addr_it = 0; addr_it < docs[i].geometry.geometries[c_it].coordinates.length; addr_it++) {
                    var lon = parseFloat(parseFloat(docs[i].geometry.geometries[c_it].coordinates[addr_it][0]).toFixed(6));
                    var lat = parseFloat(parseFloat(docs[i].geometry.geometries[c_it].coordinates[addr_it][1]).toFixed(6));
                    feat = linestring([lon, lat], vectorProperties(docs[i].properties));
                    feat.id = docs[i].id;
                    full.vectors.push(feat);
                }
            }
...
function vectorProperties(properties, point) {
    ...
    for (var k in properties) {
        ...
        case 'carmen:center':
            ...
            break;
        case 'carmen:score':
            if (properties['carmen:score'] == -1) {
                break
            } else {
                vprops['carmen:score'] = properties['carmen:score'];
            }
            break;
